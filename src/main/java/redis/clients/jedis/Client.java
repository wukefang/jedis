...
public class Client extends BinaryClient implements Commands {
    ...
    public void ehexpireAt(final String key, final String field, final long unixTime) {
        ehexpireAt(SafeEncoder.encode(key), SafeEncoder.encode(field), unixTime);
    }
    ...
    public void ehset(String key, String field, String value) {
        ehset(SafeEncoder.encode(key), SafeEncoder.encode(field), SafeEncoder.encode(value));
    }
    ...
    public void ehget(final String key, final String field) {
        ehget(SafeEncoder.encode(key), SafeEncoder.encode(field));
    }
    ...
    public void ehsetex(String key, String field, String value, int seconds) {
        ehsetex(SafeEncoder.encode(key), SafeEncoder.encode(field), SafeEncoder.encode(value), seconds);
    }

    public void ehincrBy(String key, String field, long value) {
    ...
    public void ehvals(String key) {
        ehvals(SafeEncoder.encode(key));
    }

    public void ehgetAll(String key) {
        ehgetAll(SafeEncoder.encode(key));
    }

    public void ehsetnx(String key, String field, String value) {
        hsetnx(SafeEncoder.encode(key), SafeEncoder.encode(field), SafeEncoder.encode(value));
    }

    public void ehexpire(String key, String field, int seconds) {
        ehexpire(SafeEncoder.encode(key), SafeEncoder.encode(field), seconds);
    }

    public void ehttl(String key, String field) {
        ehttl(SafeEncoder.encode(key), SafeEncoder.encode(field));
    }

    public void ehexists(String key, String field) {
        ehexists(SafeEncoder.encode(key), SafeEncoder.encode(field));
    }

    public void ehdel(String key, String... fields) {
        ehdel(SafeEncoder.encode(key), SafeEncoder.encodeMany(fields));
    }

    public void ehlen(String key) {
        ehlen(SafeEncoder.encode(key));
    }

    public void ehstrlen(String key, String field) {
        ehstrlen(SafeEncoder.encode(key), SafeEncoder.encode(field));
    }

    public void ehmset(String key, Map<String, String> hash) {
        final Map<byte[], byte[]> bhash = new HashMap<byte[], byte[]>(hash.size());
        for (final Entry<String, String> entry : hash.entrySet()) {
            bhash.put(SafeEncoder.encode(entry.getKey()), SafeEncoder.encode(entry.getValue()));
        }
        ehmset(SafeEncoder.encode(key), bhash);
    }

    public void ehmget(String key, String[] fields) {
        ehmget(SafeEncoder.encode(key), SafeEncoder.encodeMany(fields));
    }

    public void ehmsetex(String key, Jedis.TemporaryEntry... temporaryEntries) {
        final List<byte[]> keyAndEntries = new ArrayList<byte[]>();
        keyAndEntries.add(SafeEncoder.encode(key));
        for (final Jedis.TemporaryEntry temporaryEntry : temporaryEntries) {
            keyAndEntries.add(SafeEncoder.encode(temporaryEntry.field));
            keyAndEntries.add(SafeEncoder.encode(temporaryEntry.value));
            keyAndEntries.add(toByteArray(temporaryEntry.seconds));
        }
        ehmsetex(keyAndEntries);
    }
